...
(function () {
  ...
  function initRentVsBuy() {
    ...
    form.addEventListener('submit', (event) => {
      ...
      setResult(form, `<h2>${yearsHeld}-year cash-flow comparison</h2><p class="result-lede">Based only on the assumptions entered, buying has ${difference > 0 ? 'a higher' : difference < 0 ? 'a lower' : 'the same'} estimated cash outlay by ${money(Math.abs(difference))} over ${yearsHeld} ${yearWord}.</p><div class="result-grid">${card('Buying scenario', [['Up-front down payment',money(down)],['Entered closing costs',money(closing)],['Initial all-in monthly cost',money(owner.total)],['Cumulative cash paid',money(ownerCash)],['Mortgage principal paid',money(amortized.principalPaid)],['Estimated loan balance',money(amortized.balance)]])}${card('Renting scenario', [['Initial move-in cash',money(value(form, '[name="rental-move-in"]'))],['Initial rent + renter insurance',money(rent + renterInsurance)],['Annual rent increase entered',pct(rentIncrease * 100)],['Cumulative cash paid',money(rentingCash)],['Difference versus buying',signedMoney(rentingCash - ownerCash)]])}</div><p class="result-note">This is a cash-flow comparison, not an investment return. It excludes property value changes, resale costs, tax effects, investment returns, loan approval, maintenance surprises, landlord responsibilities, and non-financial reasons to rent or buy.</p>`, 'warning');
    });
  }
